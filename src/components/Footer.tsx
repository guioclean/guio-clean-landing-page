          ...
          <div className="bg-primary-foreground/5 p-10 rounded-3xl backdrop-blur-sm border border-primary-foreground/10 shadow-2xl">
            ...
            {status === "success" ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <input type="text" placeholder="Seu nome completo" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputClass} />
                
                <div className="grid sm:grid-cols-2 gap-5">
                  <input type="email" placeholder="Seu e-mail" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className={inputClass} />
                  <select required value={form.region} onChange={(e) => setForm({ ...form, region: e.target.value })} className={`${inputClass} appearance-none cursor-pointer ${form.region === "" ? "text-primary-foreground/40" : ""}`}>
                    <option value="" disabled>Selecione a Região</option>
                    {regions.map(r => <option key={r} value={r} className="text-foreground">{r}</option>)}
                  </select>
                </div>

                <textarea placeholder="Como podemos te ajudar?" required rows={4} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className={`${inputClass} resize-none`} />
                
                {status === "error" && (
                  <p className="text-destructive text-sm">Erro ao enviar. Tente novamente ou chame no WhatsApp.</p>
                )}
          ...
          <div className="flex flex-col justify-center space-y-12">
            ...
            <div className="pt-10 border-t border-primary-foreground/10 flex flex-col sm:flex-row justify-between items-center gap-6">
              <div className="flex gap-4">
                <a href="#" className="w-12 h-12 rounded-full bg-primary-foreground/10 flex items-center justify-center hover:bg-primary hover:text-primary-foreground transition-all duration-300 hover:-translate-y-1">
